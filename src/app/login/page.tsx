...
export default function LoginPage() {
  const { user, signInWithGoogle, loading } = useAuth(); // Obtém o usuário, função de login e estado de carregamento do contexto
  const router = useRouter(); // Instância do roteador Next.js

  // Efeito para redirecionar o usuário para a página principal se já estiver logado
  useEffect(() => {
    if (!loading && user) {
      router.push('/'); // Redireciona para a página inicial (principal)
    }
  }, [user, loading, router]);
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // O redirecionamento será tratado pelo useEffect acima após a atualização do estado do usuário
    } catch (error) {
  ...
  if (user) {
    return null; // Ou um spinner/mensagem de redirecionamento
  }
      ...
      <div className="bg-gray-800 p-8 md:p-12 rounded-xl shadow-2xl w-full max-w-md transform transition-all hover:scale-105 duration-300">
        ...
        <div className="mx-auto mb-8 w-24 h-24 bg-gradient-to-tr from-blue-500 to-purple-600 rounded-full flex items-center justify-center shadow-lg">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {/* Ícone genérico simples. Pode ser substituído por uma imagem ou SVG mais elaborado. */}
          </svg>
        </div>
        ...
        <div className="mb-6">
          ...
          <input 
            ...
            // value e onChange seriam necessários para login com email/senha
          />
        </div>
        ...
        {/* Botão de Login (UI apenas, funcionalidade não implementada nesta etapa) */}
        <button 
          type="button" 
          ...
          // onClick para login com email/senha seria adicionado aqui
        >
          Entrar
        </button>
        ...
        {/* Link para Registrar (Placeholder) */}
